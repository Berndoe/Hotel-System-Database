...
<?php
		...
		//write sql
		$sql1 = "SELECT MONTHNAME(paymentDate) as Months, COUNT(bk.guestId) as numberOfGuests, SUM(Charge) as roomCharge, SUM(additionalCharge) as additionalCharges,
		SUM(amountCharged) as serviceCharge FROM Payment p JOIN Booking bk on p.bookingId = bk.bookingId JOIN branchservicebooking bsb on bsb.bookingId = bk.bookingId
		JOIN services s on s.serviceId = bsb.serviceId GROUP BY MONTH(paymentDate) ORDER BY MONTH(paymentDate)";
	...
	max-width:900px;margin-left:20%; margin-top: 10%;"></canvas>
	...
	var yValues = <?php echo json_encode($guest_number); ?>;
	var yValues1 = <?php echo json_encode($roomCharge); ?>;
	var yvalues2 = <?php echo json_encode($additionalCharges); ?>;
	var yvalues3 = <?php echo json_encode($serviceCharge); ?>;
...
var myBarChart = new Chart(ctx, {
    ...
    options: {
		...
		title: {
		display: true,
		text: "Monthly Report Overnight Guests",
		fontSize: 25	
	}
}	
});
